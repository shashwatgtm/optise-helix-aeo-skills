...
What this plugin contains: Claude skills (Markdown), reference files, and one Python script, `fetch_page.py`, which fetches only the http or https address the user gives it. The plugin has no server, no hooks and no MCP server, and it stores no data.
